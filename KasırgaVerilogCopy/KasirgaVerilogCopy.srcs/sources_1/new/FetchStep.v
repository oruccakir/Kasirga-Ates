...
reg fetch_working_info_next = 1'b0; // next working info for fetch step
reg [31:0] instruction_to_decode = 32'b0; // instruction that will be convetyed to decode step
reg [31:0] instruction_to_decode_next = 32'b0; // next instruction that will be convetyed to decode step
...
reg [31:0] program_counter_next = 32'h8000_0000; // next program counter to access memory, data and instructions
reg fetch_finished = 1'b0;       // flag for fetch finished info
reg fetch_finished_next = 1'b0;  // next flag for fetch finished info
...
reg [2:0] STATE_NEXT = FIRST_CYCLE; // set first state as next state
